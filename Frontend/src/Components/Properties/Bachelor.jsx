...
const Bachelor = () => {
  ...
  const navigate=useNavigate();    
      useEffect(() => {
        axios.get("http://localhost:3000/bachelor")
          .then((res) => {
            setBachelorData(res.data);
          })
          .catch((err) => {
            console.error("Error fetching family properties:", err.message);
          });
      }, []);
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold text-center mb-8 mt-20">Bachelor Properties</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {bachelorData.map((property, index) => (
